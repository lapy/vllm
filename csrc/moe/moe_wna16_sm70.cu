...
__global__ void moe_wna16_gemm_sm70_kernel(
  ...
  for (int m = 0; m < BLOCK_SIZE_M; m++) {
    ...
    if (offset_m >= num_tokens_post_pad[0]) break;  // Safety check
...
void run_moe_wna16_gemm_sm70(
    const scalar_t* input, scalar_t* output, const uint32_t* b_qweight,
    const scalar_t* b_scales, const uint32_t* b_qzeros,
    const float* topk_weights, const int32_t* sorted_token_ids,
    const int32_t* expert_ids, const int32_t* num_tokens_post_pad,
    int num_experts, int group_size, int num_token_blocks, int top_k,
    int size_m, int size_n, int size_k, int BLOCK_SIZE_M, int BLOCK_SIZE_N,
    int BLOCK_SIZE_K, int bit, bool has_zp, bool mul_topk_weight) {
  ...
  auto kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 4, 1>;
  if (bit == 4) {
    if (BLOCK_SIZE_K / group_size == 2) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 4, 2>;
    } else if (BLOCK_SIZE_K / group_size == 4) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 4, 4>;
    } else if (BLOCK_SIZE_K / group_size == 8) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 4, 8>;
    }
  } else {
    if (BLOCK_SIZE_K / group_size == 1) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 8, 1>;
    } else if (BLOCK_SIZE_K / group_size == 2) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 8, 2>;
    } else if (BLOCK_SIZE_K / group_size == 4) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 8, 4>;
    } else if (BLOCK_SIZE_K / group_size == 8) {
      kernel = moe_wna16_gemm_sm70_kernel<scalar_t, 8, 8>;
    }
  }
...
template void run_moe_wna16_gemm_sm70<half>(
    const half* input, half* output, const uint32_t* b_qweight,
    ...
    const int32_t* num_tokens_post_pad, int num_experts, int group_size,
    int num_token_blocks, int top_k, int size_m, int size_n, int size_k,
    int BLOCK_SIZE_M, int BLOCK_SIZE_N, int BLOCK_SIZE_K, int bit, bool has_zp,
    bool mul_topk_weight);
