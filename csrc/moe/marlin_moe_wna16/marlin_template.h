...
__device__ inline void scale(typename MarlinScalarType<type_id>::FragB& frag_b,
  ...
  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 700
  // SM70 has enlarged FragB (8 elements) due to ldmatrix emulation
  #pragma unroll
  for (int j = 0; j < 8; j++) {
...
__device__ inline void scale4(
  ...
  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 700
  // SM70 has enlarged FragB (8 elements) due to ldmatrix emulation
  // For act_order, we apply the same scales to all elements in groups of 2
  #pragma unroll
  for (int j = 0; j < 4; j++) {
  ...
  #pragma unroll
  for (int j = 4; j < 8; j++) {
    frag_b[j] = __hmul2(frag_b[j], s_val_3_4);
  }
  #else
