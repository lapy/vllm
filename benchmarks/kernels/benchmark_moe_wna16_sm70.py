...
"""
Benchmark SM70 WNA16 MoE kernel.

Example (GLM-4.5 MoE on V100, TP=2):
  python benchmarks/kernels/benchmark_moe_wna16_sm70.py \
...
def main(args: argparse.Namespace) -> None:
    ...
    for bm, bn, bk in product(block_ms, block_ns, block_ks):
        if bk % group_size != 0:
            continue
...
if __name__ == "__main__":
    ...
    # Reduced defaults for CUDA kernel usually (or verify against kernel cap)
    # SM70 kernels can typically handle 32, 64, 128.
    parser.add_argument("--block-n", nargs="+", type=int, default=[32, 64, 128])
    parser.add_argument("--block-k", nargs="+", type=int, default=[32, 64])
    # Warps and stages removed
